#ifndef JS3D_MATH_INTERP_H
#define JS3D_MATH_INTERP_H
...
namespace js3d {
	...
	inline T linear_interp(const T& p1, const T& p2, const Q f)
	{
		return (static_cast<Q>(1) - f) * p1 + f * p2;
	}

	template<class T, class Q>
	inline T cosine_interp(const T& p1, const T& p2, const Q f)
	{
		const Q f2 = (static_cast<Q>(1) - cosf(f * M_PI)) / static_cast<Q>(2);
		return linear_interp(p1, p2, f2);
	}
